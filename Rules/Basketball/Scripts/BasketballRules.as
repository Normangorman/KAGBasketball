...
#include "Basketball_PopulateSpawnList.as"
#include "Logging.as";
#include "Modname.as";

const u8 POST_BASKET_PAUSE_TIME = 30;

//edit the variables in the config file below to change the basics
// no scripting required!
void Config(CTFCore@ this)
{
	string configstr = "../Mods/" + getModname() + "/Rules/Basketball/basketball_vars.cfg";
	...
	this.minimum_players_in_team = cfg.read_s32("minimum_players_in_team");
...
shared class CTFCore : RulesCore
{
	...
	void Update()
	{
        ...
        if (rules.get_bool("in post basket pause")) {
		...
		{
			gamestart = getGameTime();
			rules.set_u32("game_end_time", gamestart + gameDuration + warmUpTime);
...
void Reset(CRules@ this)
{
    ...
    this.set_u8("team 0 score", 0);
    this.set_u8("team 1 score", 0);
    this.Sync("team 0 score", true);
    this.Sync("team 1 score", true);
    this.set_u32("last basket time", getGameTime());
    this.set_bool("in post basket pause", false);
}
...
void onCommand(CRules@ this, u8 cmd, CBitStream@ params) {
    if (cmd == this.getCommandID("score basket")) {
        ...
        if (this.getCurrentState() != GAME) {
            log("onCommand", "Not in GAME state so not updating scores.");
            return;
        }
        ...
        if (basketNum == 2) {
            this.set_u8("team 0 score", this.get_u8("team 0 score") + points);
        }
        else {
            this.set_u8("team 1 score", this.get_u8("team 1 score") + points);
        }
        ...
        log("onCommand", "team 0 score: " + team0Score);
        log("onCommand", "team 1 score: " + team1Score);

        this.set_u32("last basket time", getGameTime());
        this.set_bool("in post basket pause", true);

        // Set team_wins_on_end for TimeToEnd.as
        if (team0Score > team1Score)
            this.set_s8("team_wins_on_end", 0);
...
{
	Reset(this);
	this.set_s32("restart_rules_after_game_time", 12 * 30); // post game time
